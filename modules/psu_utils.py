...
color_regex = re.compile(r"(red|orange|yellow|green|blue|purple|pink|black|white|gr(a|e)y|brown)", flags=re.IGNORECASE)
...
def find_psu_in_tierlist(listing_name: str) -> list[paw.Item] | None:
    ...
    for item in tierlist:
        proper_tokens = [x for x in [cleanup(itm) for itm in [
            item.brand,
            item.series.series,
        ...
        for index, proper_token in enumerate(proper_tokens):
            ...
            if len(proper_token) <= 3:
                # this is so short that fuzzy matching is basically useless, so we will use substring matching
    ...
    if wattage_match:
        ...
        possible_matches = [
            item for item in possible_matches if item.wattage_data.min_max.min is None
